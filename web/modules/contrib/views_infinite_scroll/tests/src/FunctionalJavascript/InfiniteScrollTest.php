...
class InfiniteScrollTest extends WebDriverTestBase {
  ...
  /**
   * Test different items count.
   */
  public function testDifferentItemsCount() {
    $this->createView('click-to-load', [
      'button_text' => 'Load More',
      'automatically_load_content' => FALSE,
      'initial' => 5,
  ...
  /**
   * Test showing of button in case there is nothing to load more.
   */
  public function testNoLoadMore() {
    $this->createView('no-load-more', [
      'button_text' => 'Load More',
      'automatically_load_content' => FALSE,
      'initial' => 0,
    ], 12);
    $this->drupalGet('no-load-more');
    $this->assertNull($this->getSession()->getPage()->findLink('Load More'));
  }
