...
/**
 * Contains \Drupal\wsib_pcd_map\Form\WsibPcdImportForm.
 *    Provides interface for the translations import administration.
 */
class WsibPcdImportForm extends ConfigFormBase {
  /**
   * @var QueueFactory
   */
  ...
  protected function getEditableConfigNames() {
    return ['wsib_pcd_map.import.settings',];
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->config('wsib_pcd_map.import.settings');
    $form['actions']['#type'] = 'actions';
    $form['actions']['run_import'] = [
      '#type' => 'submit',
      '#value' => $this->t('Trigger PCD translations import'),
      '#button_type' => 'primary',
    ];
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    // saving config
    $this->config('wsib_pcd_map.import.settings')
      ->save();
  ...
  protected function importDataQueuePopulate() {
    ...
    $queue_manual = $this->queueFactory->get('wsib_pcd_import_get_manual', TRUE);
    $queue_manual->deleteQueue();
    // get manual save queue instance.
    $queue_save_manual = $this->queueFactory->get('wsib_pcd_import_save_manual', TRUE);
    $queue_save_manual->deleteQueue();
    $operations = [];
    $operations[] = ['\Drupal\wsib_pcd_map\Form\WsibPcdMapForm::getBatchOperation', ['getPcd',[]]];
    $operations[] = ['\Drupal\wsib_pcd_map\Form\WsibPcdMapForm::getBatchOperation', ['queueCreateItem', [$queue_manual, 'tr_import',]]];
    // getting data.
    $operations[] = ['\Drupal\wsib_pcd_map\Form\WsibPcdMapForm::getBatchOperation', ['queueProcessItem', [$queue_manual, 'wsib_pcd_import_get_manual']]];
    // saving data.
    $operations[] = array('\Drupal\wsib_pcd_map\Form\WsibPcdMapForm::getBatchOperation', array('queueProcessItem', array($queue_save_manual, 'wsib_pcd_import_save_manual')));
    $batch = [
      'title' => $this->t('Wsib PCD Translations Import'),
      'operations' => $operations,
      'label' => $this->t('Wsib PCD Translations Import'),
      'finished' => NULL,
    ];
    batch_set($batch);
  }
