...
/**
 * Contains estimator import form.
 */
class EstimatorImportForm extends ConfigFormBase {
   ...
   * {@inheritdoc}
   */
  public function __construct(QueueFactory $queue, QueueWorkerManagerInterface $queue_manager, FileSystem $file_system, EntityTypeManagerInterface $entityManager) {
  ...
  public function getFormId() {
    return 'wsib_premium_estimator_import_form';
  }
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $form['help'] = [
      '#type' => 'markup',
      '#markup' => $this->t('Upload Premium Estimator file.'),
    ];

    $form['file'] = [
      '#type' => 'managed_file',
      '#title' => $this->t('Premium Estimator'),
      '#description' => $this->t('Upload a CSV file please.'),
      '#size' => 20,
      '#upload_location' => 'private://estimator',
      '#upload_validators' => [
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($fid) {
      ...
      if ($file_mime_type !== 'text/csv') {
        ...
        $original_file = $this->entityManager->getStorage('file')->load($fid);
  ...
  protected function dataQueuePopulate($file_path, $fid) {
    ...
    $queue_get_manual = $this->queueFactory->get('wsib_premium_estimator_import_get_manual', TRUE);
    $queue_get_manual->deleteQueue();
    // Get manual save queue instance.
    $queue_save_manual = $this->queueFactory->get('wsib_premium_estimator_import_save_data_manual', TRUE);
    $queue_save_manual->deleteQueue();

    $operations = [];
    $operations[] = [
      '\Drupal\wsib_premium_estimator\Form\EstimatorImportForm::getBatchOperation',
      [
        'queueCreateItem',
        [
          $queue_get_manual,
    ...
    $operations[] = [
      '\Drupal\wsib_premium_estimator\Form\EstimatorImportForm::queueProcessItem',
      [
        $queue_get_manual,
        'wsib_premium_estimator_import_get_manual',
      ],
    ];
    $operations[] = [
      '\Drupal\wsib_premium_estimator\Form\EstimatorImportForm::queueProcessItem',
      [
        $queue_save_manual,
        'wsib_premium_estimator_import_save_data_manual',
      ],
    ];

    $batch = [
      'title' => $this->t('WSIB Premium Estimator Import'),
      'operations' => $operations,
      'label' => $this->t('WSIB Premium Estimator Import'),
      'finished' => NULL,
    ];
    batch_set($batch);
  }
    ...
    for ($i = $context['sandbox']['offset']; $i <= $off; $i++) {
      ...
      if (isset($item)) {
        try {
          ...
          watchdog_exception('estimator_import', $e);
        }
      }
