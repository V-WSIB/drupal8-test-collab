...
/**
 * Provides 'WSIB Employer Classification Menu' block.
 *
 * @Block(
 *   id = "wsib_menu_employer_classification_menu_block",
 *   admin_label = @Translation("Employer Classification Menu Block"),
 * )
 */
class EmployerClassificationMenuBlock extends BlockBase implements ContainerFactoryPluginInterface {
  /**
      ...
      if (!empty($child_terms)) {
        ...
        foreach ($child_terms as $child) {
          ...
          if (isset($current) && is_object($current) && ($current->getEntityTypeId() === 'taxonomy_term' && $child->id() === $current->id()) ||
            (isset($current) && is_object($current) && $current->getEntityTypeId() === 'node' && $current->getType() === 'employer_classification' && $child->id() === $current->get('field_related_subject')->target_id)) {
